...
module.exports = {
    ...
    async execute(i) {
        ...
        if (category.children.size >= 50) {
            ...
            const buttons = new ButtonBuilder().addComponents(
                new ButtonBuilder().setCustomId('Button-claim').setEmoji('✋').setLabel('Claim').setStyle(ButtonStyle.Primary),
                new ButtonBuilder().setCustomId('Button-cancel').setEmoji('❌').setLabel('Cancel').setStyle(ButtonStyle.Primary)
            );
            const emb = new EmbedBuilder().setColor('#FB8C00').setFooter({ text: `Ticket created by ${i.user.tag}`, iconURL: i.user.displayAvatarURL({ dynamic: true }) }).setDescription('A staff member will get back to you soon!').setTitle(`:ticket: Ticket #${stp.ticket + 1}`);
